...
class SiPixelRawDump_Pilot : public edm::EDAnalyzer {
public:

  /// ctor
  explicit SiPixelRawDump_Pilot( const edm::ParameterSet& cfg);

  //explicit SiPixelRawDump_Pilot( const edm::ParameterSet& cfg) : theConfig(cfg) {
  //consumes<FEDRawDataCollection>(theConfig.getUntrackedParameter<std::string>("InputLabel","source"));} 

  /// dtor
  virtual ~SiPixelRawDump_Pilot() {}
...
//----------------------------------------------------------------------------------
SiPixelRawDump_Pilot::SiPixelRawDump_Pilot( const edm::ParameterSet& cfg) : theConfig(cfg) {
...
//----------------------------------------------------------------------------------------
void SiPixelRawDump_Pilot::endJob() {
...
//----------------------------------------------------------------------
void SiPixelRawDump_Pilot::beginJob() {
...
//-----------------------------------------------------------------------
void SiPixelRawDump_Pilot::analyze(const  edm::Event& ev, const edm::EventSetup& es) {
...
//   hintgl->Fill(float(lumiBlock),float(intlumi));
  ...
  for (int fedId = fedIds.first; fedId <= fedIds.second; fedId++) {
    ...
    if(fedId != 40 ) continue; // skip nonpilot feds
	...
	int layer = Decode::checkLayerLink(fedId, i); // get bpix layer 
	if(layer>10) layer = layer-10; // ignore 1/2 modules 
...
DEFINE_FWK_MODULE(SiPixelRawDump_Pilot);
