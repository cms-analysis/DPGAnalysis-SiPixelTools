...
#include "SiPixelGenErrorDBLoader.h"

SiPixelGenErrorDBLoader::SiPixelGenErrorDBLoader(const edm::ParameterSet& iConfig):
...
SiPixelGenErrorDBLoader::~SiPixelGenErrorDBLoader() {
}

void SiPixelGenErrorDBLoader::beginJob() {
}

void SiPixelGenErrorDBLoader::analyze(const edm::Event& iEvent, const edm::EventSetup& es) {
  ...
  obj->setVersion(theVersion); // external version 
  ...
  for(int m=0; m< obj->numOfTempl(); ++m){
	...
	in_file >> tempstore;
      ...
      edm::LogError("SiPixelGenErrorDBLoader") << "Error opening File" << tempfile;
    }
  }
...
void SiPixelGenErrorDBLoader::endJob() {
}


//define this as a plug-in
DEFINE_FWK_MODULE(SiPixelGenErrorDBLoader);
