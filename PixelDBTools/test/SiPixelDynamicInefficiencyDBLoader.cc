...
#include "SiPixelDynamicInefficiencyDBLoader.h"
...
SiPixelDynamicInefficiencyDBLoader::SiPixelDynamicInefficiencyDBLoader(edm::ParameterSet const& conf) : 
  conf_(conf){
...
void SiPixelDynamicInefficiencyDBLoader::beginJob(){
  
}

// Virtual destructor needed.

SiPixelDynamicInefficiencyDBLoader::~SiPixelDynamicInefficiencyDBLoader() {  

}  

// Analyzer: Functions that gets called by framework every event

void SiPixelDynamicInefficiencyDBLoader::analyze(const edm::Event& e, const edm::EventSetup& es)
{
  ...
  for(Parameters::iterator it = thePixelGeomFactors_.begin(); it != thePixelGeomFactors_.end(); ++it) {
    ...
    else edm::LogError("SiPixelDynamicInefficiencyDBLoader")<<"SiPixelDynamicInefficiencyDBLoader input detector part is neither bpix nor fpix"<<std::endl;
  }
  ...
  if( mydbservice.isAvailable() ){
	...
	mydbservice->appendSinceTime<SiPixelDynamicInefficiency>(DynamicInefficiency,
      ...
      edm::LogError("SiPixelDynamicInefficiencyDBLoader")<<er.what()<<std::endl;
    }catch(const std::exception& er){
      edm::LogError("SiPixelDynamicInefficiencyDBLoader")<<"caught std::exception "<<er.what()<<std::endl;
    }catch(...){
      edm::LogError("SiPixelDynamicInefficiencyDBLoader")<<"Funny error"<<std::endl;
    }
  }else{
    edm::LogError("SiPixelDynamicInefficiencyDBLoader")<<"Service is unavailable"<<std::endl;
  }
   
}
void SiPixelDynamicInefficiencyDBLoader::endJob(){
}

//define this as a plug-in
DEFINE_FWK_MODULE(SiPixelDynamicInefficiencyDBLoader);
