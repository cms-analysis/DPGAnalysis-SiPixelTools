...
class PixTracksTest : public edm::EDAnalyzer {
 public:
  
  explicit PixTracksTest(const edm::ParameterSet& conf);  
  virtual ~PixTracksTest();
...
/////////////////////////////////////////////////////////////////
// Contructor,
PixTracksTest::PixTracksTest(edm::ParameterSet const& conf) 
...
PixTracksTest::~PixTracksTest() { }  

// ------------ method called at the begining   ------------
void PixTracksTest::beginRun(const edm::EventSetup& iSetup) {
  cout << "BeginRun, Verbosity =  " <<PRINT<<endl;
}

// ------------ method called at the begining   ------------
void PixTracksTest::beginJob() {
...
void PixTracksTest::analyze(const edm::Event& e, 
  ...
  for(reco::TrackCollection::const_iterator t=recTracks->begin();
	...
	dynamic_cast<const PixelGeomDetUnit*> (theTracker.idToDet(hit_detId) );
      //double detZ = theGeomDet->surface().position().z(); // unused presently
      //double detR = theGeomDet->surface().position().perp(); // unused presently
...
DEFINE_FWK_MODULE(PixTracksTest);
