...
int run_new(const char* file ){
  ...
  _file0->cd("Histos/Run251143");

  //Get resolution along x
  TH1F*  h420= (TH1F*) _file0->Get("Histos/Run251143/h420");
  h420->SetDirectory(0); // "detach" the histogram from the file
  fittp0("h420");
  float rms420 = h420->GetRMS();
  std::cout<<" rms 420 = "<< rms420<<std::endl;
 

  //Get resolution along y
  TH1F*  h421= (TH1F*) _file0->Get("Histos/Run251143/h421");
  h421->SetDirectory(0); // "detach" the histogram from the file
  fittp0("h421");
  float rms421 = h421->GetRMS();
  std::cout<<" rms 421 = "<< rms421<<std::endl;
