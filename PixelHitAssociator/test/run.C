{
  const bool lookAtAlignmentErrors = true;
  ...
  float rms = RecHit_xres_f_All.GetRMS();
  cout<<" disk x RMS = "<<rms<<endl;

  RecHit_yres_f_All.Fit("gaus");
  float rms = RecHit_yres_f_All.GetRMS();
